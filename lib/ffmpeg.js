	...
	var __constructor = function (args) {
		...
		when(_getInformation(inputFilepath, settings), function (data) {
			...
			if (typeof callback == 'function') {
				...
				callback(null, new video(inputFilepath.replace(/(["\s'$`\\])/g,'\\$1')+'"', settings, data[0], data[1]));
			} else {
				// Positive response
				deferred.resolve(new video(inputFilepath.replace(/(["\s'$`\\])/g,'\\$1')+'"', settings, data[0], data[1]));
			}
		}, function (error) {
			// Check if the callback is a function
			if (typeof callback == 'function') {
